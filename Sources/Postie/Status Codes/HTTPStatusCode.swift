...
public enum HTTPStatusCode: UInt16 {
    ...
    /// case of a request for which a body needs to be sent; for example, a POST request).
    ///
    ...
    /// that the server does not support expectations (this is the case, for example, of HTTP/1.0 servers).
    case `continue` = 100
    ...
    /// The server is a transforming proxy (e.g.a Web accelerator) that received a 200 OK from its origin, but is returning a
    ...
    ///
    /// The HTTP/1.0 specification (RFC 1945) required the client to perform a temporary redirect (the original describing phrase
    /// was "Moved Temporarily"), but popular browsers implemented 302 with the functionality of a 303 See Other.
    ...
    /// scheme, as proposed, for example, by GNU Taler, but that has not yet happened, and this code is not widely used.
    ///
    ...
    /// This code is also typically used if the request provided authentication by answering the
    /// WWW-Authenticate header field challenge, but the server did not accept that authentication.
    ...
    /// The server is unwilling to process the request because either an individual header field, or all the header fields collectively,
    /// are too large.
    ///
    /// (RFC 6585)
